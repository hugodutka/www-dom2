...
export const QuizSummary = ({
  ...
  saveScoreNoStats, saveScoreWithStats, exit
}: {
  quiz: { title: string, description: string}, score: number,
  questions: { [id: string]: { [ question: string]: string } },
  questionsOrder: Array<string>, userAnswers: { [id: string]: string },
  correctAnswers: { [id: string]: boolean }, saveScoreNoStats(): void, saveScoreWithStats(): void,
  exit(): void
}) => (
      ...
      <div>
        <h5>
          ...
          <span className={`badge badge-${correctAnswers[id] ? "success" : "danger"}`}>
            Odpowiedź {correctAnswers[id] ? "poprawna" : "błędna"}
          </span>
        </h5>
